...
## 107 · Deploy a draft phase now, without scheduling it — ✅ LIVE 21 Sep

- [ ] **"Deploy now" is the main button on a draft phase** on Nutrition, Measurements and Training (it was "Schedule it", which opened the dates dialog). One confirm, in the phase dialog's look, says what happens, then it is live: it starts this week and keeps its length. "Schedule" sits beside it for going out later. "Make it live" on a scheduled nutrition or lifestyle phase uses the same confirm instead of the dates dialog. On the Plan tab, a draft that starts later gets "Put it live now" next to "Schedule for …".
- [ ] **The phase running on that track makes room**, and the confirm says so: it ends last week, or, if it only started this week, it goes back to a draft. A nutrition or lifestyle phase put live now runs up to the next phase already set on the track rather than overlapping it ("fgh takes over on 5 Oct, as planned"). A programme keeps its weeks; the programme it replaces becomes a past one, and a programme scheduled later still takes over on its date.
...
## 109 · The demo video dialog in the new look — ✅ LIVE 21 Sep
...
## 111 · A scheduled programme's start can be moved in the phase dialog — ✅ LIVE 21 Sep
...
## 124 · Measurements: "Saved" after a change to the tracked metrics — ✅ LIVE 21 Sep
...
6. **A coach’s content feed for their clients** (idea, 22 Sep). Beside the coach profile (group 48): a feed where the coach posts videos and short pieces (nutrition explainers, technique, mindset) and their clients browse them from the app, like a small library or forum. Open questions: where it lives on the client side (the Coach row on Account, or its own tab), whether posts are for all of a coach’s clients or can be aimed at some, whether clients can comment, and video hosting (uploads bucket vs. YouTube/Vimeo links).
