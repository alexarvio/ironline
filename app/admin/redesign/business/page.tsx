...
export default async function BusinessPage() {
  ...
  const genderSplit = (["Male", "Female", "Other"] as const).map((g) => ({ label: g, count: genders.filter((x) => x === g).length })).filter((g) => g.count > 0);
  ...
  const people: { label: string; value: string; unit?: string; basis: string }[] = [
    ...
    { label: "Longest", value: tenures.length ? span(Math.max(...tenures)) : "–", basis: "since their coaching start" },
  ];
  const revenue: { label: string; value: string; basis: string; tone?: "warn" }[] = [
    ...
    { label: "Per client per month", value: avg(monthly) != null ? money(avg(monthly)!) : "–", basis: basis(monthly.length) },
    { label: "Outstanding", value: money(open), basis: "invoices not marked paid", tone: open > 0 ? "warn" : undefined },
  ];
  const byPaid = [...clients].sort((a, b) => b.paid - a.paid || a.name.localeCompare(b.name));
      ...
      <div className="rd-page">
        <div className="bzd">
          <header>
            ...
            <p className="bzd-sub">From what is filled in on each client&rsquo;s card; blanks are left out, never counted as zero.</p>
          </header>
            ...
            <Card>
              ...
              <CardContent>
                {genderSplit.length ? (
                      ...
                      return (
                        <div key={g.label} className="bzd-bar-row">
                          ...
                          <span className="bzd-bar-track">
                            <span className="bzd-bar-fill" style={{ width: `${pct}%` }} />
                          </span>
                  ...
                  <p className="bzd-empty">No gender filled in on any card yet.</p>
                )}
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                {ages.length ? (
                  <div className="bzd-bars">
                    {bands.map((b) => (
                      <div key={b.label} className="bzd-bar-row">
                        ...
                        <span className="bzd-bar-track">
                          <span className="bzd-bar-fill" style={{ width: `${(b.count / bandMax) * 100}%` }} />
                        </span>
                  ...
                  <p className="bzd-empty">No birthdate filled in on any card yet.</p>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          <section className="bzd-section" aria-label="Revenue">
            ...
            <div className="bzd-tiles four">
              {revenue.map((t) => (
                <Card key={t.label} className={`bzd-tile${t.tone ? ` ${t.tone}` : ""}`}>
                  <CardContent>
                    <span className="bzd-label">{t.label}</span>
                ...
                <Table className="bzd-table">
                  ...
                  <TableBody>
                    {byPaid.map((c) => (
                      <TableRow key={c.id}>
                        <TableCell>
                          <Link href={`/admin/redesign/home?client=${c.id}`} className="bzd-name">
                            {c.name}
                          </Link>
                        </TableCell>
